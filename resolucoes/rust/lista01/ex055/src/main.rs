...
fn main() {
    loop {
        ...
        let allowed_range = 1..=10;

        let mut rng = rand::thread_rng();
        let random_number = rng.gen_range(allowed_range.clone());

        let mut chances = 4;

        loop {
            let allowed_values: Vec<i32> = allowed_range.clone().collect();
            let choice = get_int("a sua escolha entre 1 e 10", Some(allowed_values), false);
            ...
            if choice == random_number {
                ...
                if chances == 0 {
                    println!("Você perdeu, o número era {}.", random_number);
                    break;
                }

                println!("Você errou, tente novamente. Você tem {} chances.", chances);
            }
        }
